...
async function run() {
  ...
  // ═══════════════════════════════════════════════════════════════
  // 1. Leads page — screenshot of lead cards
  // ═══════════════════════════════════════════════════════════════
  console.log('\n1. Lead Sniper Feed...');
  await page.goto(`${BASE}/dashboard/leads`, { waitUntil: 'networkidle', timeout: 20000 });
  await page.waitForTimeout(5000);

  // Scroll to where the lead cards are
  const leadCard = page.locator('button:has-text("צפה")').first();
  ...
  // ═══════════════════════════════════════════════════════════════
  // 2. Click "צפה" on the Instagram lead (should open modal)
  // ═══════════════════════════════════════════════════════════════
  console.log('\n2. Opening lead modal...');

  // Find all "צפה" buttons and check their parent card platform
  const allButtons = page.locator('button:has-text("צפה")');
  const btnCount = await allButtons.count();
  console.log(`   Total "צפה" buttons: ${btnCount}`);

  // Click each button until we find one that opens the modal
  let modalOpened = false;
  for (let i = 0; i < btnCount && !modalOpened; i++) {
    ...
    // Check if modal appeared (z-50 fixed overlay)
    ...
    if (modalVisible) {
      ...
      console.log(`   → Button ${i} did not open modal (external URL?)`);
    }
  }

  if (!modalOpened) {
    console.log('   ✗ No modal opened from any button');
    // Screenshot current state for debugging
    await page.screenshot({ path: `${SCREENSHOTS_DIR}/7-lead-modal-debug.png`, fullPage: false });
  }

  // ═══════════════════════════════════════════════════════════════
  // 3. Market Intelligence (fresh page load)
  // ═══════════════════════════════════════════════════════════════
