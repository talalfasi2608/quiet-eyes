...
async function run() {
  ...
  // Also click Google Maps lead (3rd button)
  if (count >= 3) {
    await viewButtons.nth(2).scrollIntoViewIfNeeded();
    await viewButtons.nth(2).click();
    await page.waitForTimeout(1500);
    await page.screenshot({ path: `${SCREENSHOTS_DIR}/7b-lead-modal-maps.png`, fullPage: false });
    console.log('   ✓ Modal screenshot (google maps lead)');
  ...
  await page.waitForTimeout(500);
  await page.screenshot({ path: `${SCREENSHOTS_DIR}/8b-market-intelligence-scroll.png`, fullPage: false });
